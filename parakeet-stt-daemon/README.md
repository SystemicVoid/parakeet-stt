...
- Run the server for CPU profile parity (`stt cpu`): `PARAKEET_DEVICE=cpu PARAKEET_STREAMING_ENABLED=false uv run parakeet-stt-daemon`
...
- Config overrides use `PARAKEET_` env vars (e.g., `PARAKEET_SHARED_SECRET`, `PARAKEET_MIC_DEVICE`); status endpoint is enabled by default, disable with `--no-status`. When VAD is enabled, `/status` now reports `vad_enabled`, `vad_active`, and `vad_fallback_reason`.
