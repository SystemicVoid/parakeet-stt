...
- Personal eval tooling (local-only corpus): use repo-root `just` entry points (`just eval-candidates`, `just eval-materialize`, `just eval-record`, `just eval-calibrate-offline`, `just eval-daily-offline`, `just eval-calibrate-stream`, `just eval-daily-stream`, `just eval-compare`). The recorder keeps defaults (`bench_audio/personal/manifest.jsonl` -> `bench_audio/personal/audio`) and key controls (`s`, `r`, `n/p`, `q`).
