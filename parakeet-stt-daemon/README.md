...
- Personal eval tooling (local-only corpus): `scripts/build_personal_eval_candidates.py`, `scripts/materialize_personal_manifest.py`, `scripts/record_personal_clips.sh`, then `uv run python check_model.py --bench-offline --bench-manifest bench_audio/personal/manifest.jsonl --bench-tier daily --baseline bench_audio/personal/baseline.json`
