...
def ensure_chrome():
    try:
        ...
        pass
    subprocess.Popen(["google-chrome", "--headless=new", "--disable-gpu", "--hide-scrollbars",
                      "--window-size=1920,1080", f"--remote-debugging-port={PORT}",
                      "--user-data-dir=/tmp/overlay-proto-chrome", "about:blank"],
...
async def main(specs):
    ...
    async with websockets.connect(page["webSocketDebuggerUrl"], max_size=64 * 1024 * 1024) as ws:
        ...
        for spec in specs:
            ...
            v, _, q = v.partition("?")
            url = f"http://localhost:8765/?variant={v}&backdrop={bd}&auto={scn}" + (("&" + q) if q else "")
            tag = v + ("-" + q.split("=")[-1] if q else "")
            await call("Page.navigate", url=url)
            deadline = time.time() + 40
            while time.time() < deadline:
                r = await call("Runtime.evaluate", expression="window.OverlayProto ? OverlayProto.state.phase : ''", returnByValue=True)
